...
session.cookies['sessionid'] = 'ayg20cd2rl8rl5xemu2bl3z61yk8xkzb'
...
def get_one_sum(info, md):
	...
	sum_data = 0
	for td in tds:
		infos = re.findall('<img src="(.*?)" class="img_number (.*?)" style="left:(.*?)px">', td)
		# print(md)
		for info in infos:
			print('info: ', info[1])
			if info[1] != md:
				img_li.append(num[info[0]])
				iv_li.append(int(float(info[2]) / 11.5))

		# print(img_li)
		# print(iv_li)
		# print(num_li)
		for i, item in enumerate(iv_li):
			# print(num_li[i])
			# print(img_li[i])
			num_li[i] = img_li[i + (num_li[i] + iv_li[i])]
		# print('=' * 50)
		# print(num_li)
		# print('-' * 200)
		sum_data += int(''.join(str(x) for x in num_li))
		img_li.clear()
		iv_li.clear()
		num_li = [0] * 4
	all_data.append(sum_data)


for i in range(1, 6):
	url = f'https://match.yuanrenxue.cn/api/match/4?page={i}'
